...
def discovery(sites = discoverySites, driver_iwait: int = 30, scrollOffset: int = 50):
  # mongodb connection
  try:
    db = dbase.init_db(uri=MONGODB_URI, database=MONGODB_DATABASE)
  except:
  ...
  for site in sites:
    try:
      wdriver.discover_site(db = db, driver = driver, site = site, maxPage = DISCOVERY_MAXPAGES, scrollOffset = scrollOffset)
    except:
      continue
