...
for rootdir, subdirs, files in os.walk(importdir):
  for moviefile in files:
    ...
    if os.path.splitext(moviefile)[1] in [".mkv", ".mp4"]:
      ...
      nfochannel = nfotitle.replace('_','.').split('.')[0].lower()
      date = None
      movieid = None
      actor = None
      ...
      try:
        date = '.'.join(map(str, nfotitle.replace('_','.').split('.')[1:4]))
        ...
        pass
      try:
        actor = nfotitle.replace('_','.').split('.')[4]
        #print("actor:", actor)
      except:
      ...
      if (date is None):
        try:
          movieid = nfotitle.replace('_','.').lower().split('.', 1)[1].split('.xxx')[0].split('.fullhd')[0].split('.hd')[0].replace('.', ' ')
